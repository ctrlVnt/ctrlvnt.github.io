...
const Links = () => {
  ...
  return (
    <section id="links" className="relative py-16 bg-white overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* Floating Nodes */}
        <div 
          className="absolute top-24 left-24 w-3 h-3 bg-blue-400/40 rounded-full"
          style={{ transform: `translate(${Math.sin(scrollY * 0.01) * 20}px, ${Math.cos(scrollY * 0.01) * 15}px)` }}
        />
        <div 
          className="absolute top-40 right-32 w-4 h-4 bg-indigo-400/30 rounded-full"
          style={{ transform: `translate(${Math.cos(scrollY * 0.008) * 25}px, ${Math.sin(scrollY * 0.008) * 18}px)` }}
        />
        <div 
          className="absolute bottom-48 left-1/3 w-2 h-2 bg-purple-400/50 rounded-full"
          style={{ transform: `translate(${Math.sin(scrollY * 0.012) * 30}px, ${Math.cos(scrollY * 0.012) * 12}px)` }}
        />
        <div 
          className="absolute bottom-32 right-20 w-5 h-5 bg-blue-300/25 rounded-full"
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Essential resources for students, researchers, and mathematics enthusiasts
          </p>
        </div>
            ...
            {quickLinks.map((link, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-medium text-gray-900 group-hover:text-blue-600">{link.name}</h4>
